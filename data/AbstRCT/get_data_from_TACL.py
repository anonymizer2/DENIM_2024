...
def get_data(dataset_type):
    mrp_file_path = './data/AbstRCT/abstrct_' +dataset_type+ '.mrp'  # 请替换为你的文件路径
    output_json_file_path = './data/AbstRCT/abstrct_' + dataset_type+ '.json'
    ...
    ac_label_dict = {'AbstRCT_MajorClaim': 'MajorClaim',
                    'AbstRCT_Claim': 'Claim',
                    'AbstRCT_Evidence': 'Evidence',
    }

    ar_label_dict = {'AbstRCT_Support': 'Support',
                    'AbstRCT_Partial-Attack': 'Partial-Attack',
                    'AbstRCT_Attack': 'Attack'
    }
    ...
    for a_data in dataset:
        ...
        dropped_id = {}
        if len(para_text.split(' ')) > max_len_for_bart or len(adu_spans) >= 11: 
            exceeded_idx = len(adu_spans)
            for i, span in enumerate(adu_spans):
                if span[0] > max_len_for_bart or span[1] > max_len_for_bart or i >= 11:
