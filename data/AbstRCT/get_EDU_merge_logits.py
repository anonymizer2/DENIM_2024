...
def get_data(data_kind):
    ...
    with open('./data/AbstRCT/abstrct_'+data_kind+'.json', 'r') as file:
        data_Ab = json.load(file)
    with open('./data/AbstRCT/abstrct_RST_logits_' + data_kind + '.json', 'r') as file:
        data = json.load(file)
    ...
    with open('./data/AbstRCT/abstrct_EDU_logits_'+data_kind+'.json', 'w') as file:
        json.dump(data_Ab_new, file, indent=4, ensure_ascii=False)
